...
const char* GetFullBankPath(const char* BankName)
{
	...
	Q_snprintf(resultpath, FILESYSTEM_MAX_SEARCH_PATHS, "sound/fmod/%s", BankName);
...
FMOD::Studio::Bank* CFMODManager::LoadBankFile(Studio::Bank* bank, const char* FileName, bool loadfromvpk)
{
	FMOD_RESULT result;
	const char* path = GetFullBankPath(FileName);
	if (loadfromvpk)
	{
		//load a bank file from a vpk
		//-Nbc66
		char fullpath[512];
		V_snprintf(fullpath, sizeof(fullpath), "sound/fmod/%s", FileName);

		void* buffer = NULL;

		int length = filesystem->ReadFileEx(fullpath, "GAME", &buffer, true, true);
		result = pStudio_System->loadBankMemory((const char*)buffer, length, FMOD_STUDIO_LOAD_MEMORY, FMOD_STUDIO_LOAD_BANK_NORMAL, &bank);

		//if we fail to load a bank file from a vpk
		//default to loading it as a loos file
		//if (result != FMOD_OK)
		//{
...
void TestFmodEvent(const CCommand& args)
{
	...
	result_studio = pStudio_System->getEvent("{74aee94a-c997-485d-8f6d-ee67a301e673}", &events);
...
{
	result_studio = envinstance->setParameterByName("switch", atoi(args.Arg(1)));
