...
class C_NPC_Manblower : public C_AI_BaseNPC
{
public:
	DECLARE_CLASS( C_NPC_Manblower, C_AI_BaseNPC );
	DECLARE_CLIENTCLASS( );
	DECLARE_DATADESC( );
...
private:
	C_NPC_Manblower( const C_NPC_Manblower & );
...
// Save/restore
//-----------------------------------------------------------------------------
BEGIN_DATADESC( C_NPC_Manblower )
END_DATADESC( )

//-----------------------------------------------------------------------------
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT( C_NPC_Manblower, DT_NPC_Manblower, CNPC_Manblower )
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Manblower::OnDataChanged( DataUpdateType_t type )
{
	...
	{
		SoundInit( );
		if( m_pEngineSound1 != NULL && m_pEngineSound2 != NULL )
		{
...
// Restore
//-----------------------------------------------------------------------------
void C_NPC_Manblower::OnRestore( )
{
	BaseClass::OnRestore( );
	SoundInit( );
}
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Manblower::UpdateOnRemove( )
{
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Manblower::SoundInit( )
{
	if( m_nEnginePitch1 < 0 || m_nEnginePitch2 < 0 )
		return;
	...
	if( m_pEngineSound1 == NULL )
	{
		m_pEngineSound1 = CSoundEnvelopeController::GetController( ).SoundCreate( filter, entindex( ), "NPC_SScanner.FlyLoop" );
...
void C_NPC_Manblower::SoundShutdown( )
{
	// Kill the engine!
	if( m_pEngineSound1 != NULL )
	{
