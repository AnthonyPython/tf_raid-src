		...
		if ( pLocalPlayer )
		{
			...
			if ( pPlayer && pPlayer != pLocalPlayer )
			{
				...
				if ( TFGameRules() && TFGameRules()->IsMannVsMachineMode() && ( pPlayer->GetTeamNumber() == TF_TEAM_PVE_INVADERS ) )
					return;
				if (TFGameRules() && TFGameRules()->IsRaidMode() && (pPlayer->GetTeamNumber() == TF_TEAM_PVE_DEFENDERS))
					return;
...
void CTFSpectatorGUI::RecalculatePlayerPanels( void )
{
	...
	bool bMvM = TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode();
...
void CTFSpectatorGUI::UpdatePlayerPanels( void )
{
	...
	// We only want to draw the player panels for the blue team in Raid
	if (TFGameRules() && TFGameRules()->IsRaidMode())
	{
		iTeam1 = TF_TEAM_PVE_INVADERS;
		iTeam2 = TF_TEAM_PVE_DEFENDERS;
