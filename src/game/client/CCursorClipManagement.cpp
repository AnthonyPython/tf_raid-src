...
// Cyanide; me when don't know if checking convar per-frame is slow
void DisableCursorClippingChanged( IConVar *pConVar, const char *oldString, float flOldValue )
{
	if( !g_pCursorClipManager )
		return;

	ConVarRef var( pConVar );
	g_pCursorClipManager->SetDisabledWhen( var.GetInt() );
}
ConVar cl_disable_cursor_clipping( "cl_disable_cursor_clipping", "1", FCVAR_ARCHIVE | FCVAR_DONTRECORD, 
	"Disable cursor clipping. 1 - Disabled, 2 - Just disabled in menus", DisableCursorClippingChanged );
...
void CCursorClipManagement::Think()
{
	...
	if( m_nDisabledWhen == 1 || ( m_nDisabledWhen == 2 && enginevgui->IsGameUIVisible() ) )
	{
		UnlockCursor();
		return;
	}
...
void CCursorClipManagement::Init(void)
{
	...
	g_pCursorClipManager->SetDisabledWhen( cl_disable_cursor_clipping.GetInt() );
}
