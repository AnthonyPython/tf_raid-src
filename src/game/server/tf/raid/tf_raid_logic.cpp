...
CTFBot* SpawnRedTFBot(int botClass, const Vector& rawSpot, bool is_rusher = false)
{
	...
	if (bot && bot->IsAlive())
	{
		if (botClass >= TF_CLASS_SCOUT && botClass <= TF_CLASS_ENGINEER)
		{
			if (( bot->IsMiniBoss()) && g_pFullFileSystem->FileExists(g_szBotBossModels[botClass]))
			{
				bot->GetPlayerClass()->SetCustomModel(g_szBotBossModels[botClass], USE_CLASS_ANIMATIONS);
				bot->UpdateModel();
				bot->SetBloodColor(DONT_BLEED);
			}
			else if (g_pFullFileSystem->FileExists(g_szBotModels[botClass]))
			{
				bot->GetPlayerClass()->SetCustomModel(g_szBotModels[botClass], USE_CLASS_ANIMATIONS);
				bot->UpdateModel();
				bot->SetBloodColor(DONT_BLEED);
			}
		}
	...
	{
		bot->ModifyMaxHealth(tf_raid_wandering_max_health.GetInt());
		bot->SetMaxSpeed(tf_raid_wandering_max_speed.GetFloat());
	}
...
bool CRaidLogic::SpawnSquad( CTFNavArea *spawnArea )
{
	...
	//squadClasses[ squadClassCount++ ] = TF_CLASS_SNIPER;
	//squadClasses[ squadClassCount++ ] = TF_CLASS_ENGINEER;
	...
	for( int i=0; i<squadSize; ++i )
	{
		...
		bot = SpawnRedTFBot( which, spawnArea->GetCenter(),true );
		if ( !bot )
			return false;
...
void CRaidLogic::SpawnEngineers( void )
{
	...
	while( engineerCount < tf_raid_max_defense_engineers.GetInt() )
	{
		...
		{
			DevMsg( "RAID: %3.2f: Can't spawn engineer spawning just any where!", gpGlobals->curtime );
			CTFBot* bot = SpawnRedTFBot(TF_CLASS_ENGINEER, sentryArea->GetParent()->GetCenter() + Vector(0, 0, RandomFloat(0.0f, 30.0f)));
			if (bot)
			{
				++engineerCount;
