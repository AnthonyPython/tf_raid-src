...
IMPLEMENT_SERVERCLASS_ST( CTFPlayer, DT_TFPlayer )
	...
	SendPropBool(SENDINFO(m_bIsSpecial)),
...
bool CTFPlayer::CanBeForcedToLaugh( void )
{
	...
	if (TFGameRules() && TFGameRules()->IsRaidMode() && IsBot() && (GetTeamNumber() == TF_TEAM_PVE_DEFENDERS))
		return false;
			...
			for ( int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++ )
			{
				// bots don't need the action slot item for MvM (canteen)
 				if ( ( i == LOADOUT_POSITION_ACTION ) && IsBot() && TFGameRules() && TFGameRules()->IsMannVsMachineMode() && ( GetTeamNumber() == TF_TEAM_PVE_INVADERS ) )
 					continue;
				// bots don't need the action slot item for Raid (canteen)
				if ((i == LOADOUT_POSITION_ACTION) && IsBot() && TFGameRules() && TFGameRules()->IsRaidMode() && (GetTeamNumber() == TF_TEAM_PVE_DEFENDERS))
					continue;
...
//	UpdateSkillRatingData();
	...
	bool bIsMvMRobot = (TFGameRules()->IsMannVsMachineMode() ) || (TFGameRules()->IsRaidMode() ) && IsBot();
...
// 		}
	...
	{
		//which team?
		int team = (TFGameRules()->IsMannVsMachineMode() ? TF_TEAM_PVE_DEFENDERS : TF_TEAM_PVE_INVADERS);
		...
		if ( GetTeamNumber() == team)
		{
			...
			else
			{
				...
				if (TFGameRules()->IsRaidMode())
				{
					TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_DEFENDER_DIED, TF_TEAM_PVE_INVADERS, CFmtStr("victimclass:%s", g_aPlayerClassNames_NonLocalized[GetPlayerClass()->GetClassIndex()]).Access());
				}
				else
				{
	...
	if ( !TFGameRules()->IsMannVsMachineMode() && !(TFGameRules()->IsRaidMode() )|| !IsBot() )
	{
...
bool CTFPlayer::SetObserverMode(int mode)
{
	...
	bool bAllowSpecModeChange = TFGameRules()->IsInTournamentMode() ? (TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode()): true;
...
void CTFPlayer::DeathSound( const CTakeDamageInfo &info )
{
	...
	if (TFGameRules() && TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)
	{
		nDeathSoundOffset = IsMiniBoss() ? DEATH_SOUND_GIANT_MVM_FIRST : DEATH_SOUND_MVM_FIRST;
	}
	...
	if (TFGameRules() && TFGameRules()->IsRaidMode() &&
		GetTeamNumber() != TF_TEAM_PVE_DEFENDERS && !m_bGoingFeignDeath)
	{
		EmitSound("MVM.PlayerDied");
		return;
	}
	...
	if ( TFGameRules() && (TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode() ) && IsMiniBoss() )
	{
		switch ( GetPlayerClass()->GetClassIndex() )
		{
			case TF_CLASS_HEAVYWEAPONS:
...
const char* CTFPlayer::GetSceneSoundToken( void )
{
	...
	else if(TFGameRules() && TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)
	{
		if (IsMiniBoss())
		{
			return "M_MVM_";
...
bool CTFPlayer::IsValidObserverTarget( CBaseEntity * target )
{
	...
	if ( !target->IsPlayer() )
	{
		...
		{
			bStrictRules = ( TFGameRules()->IsInTournamentMode() && !TFGameRules()->IsMannVsMachineMode() && !TFGameRules()->IsRaidMode());
		}
...
//=============================================================================
	...
	//We'll use mvm stuff for now. change later
	if (TFGameRules()->IsRaidMode())
	{
		if (GetTeamNumber() == TF_TEAM_BLUE)
		{
