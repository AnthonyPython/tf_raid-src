...
void CTFBotVision::UpdatePotentiallyVisibleNPCVector( void )
{
	if ( m_potentiallyVisibleUpdateTimer.IsElapsed() )
	{
		...
		bool bShouldSeeTeleporter = !TFGameRules()->IsMannVsMachineMode() || !TFGameRules()->IsRaidMode() || GetBot()->GetEntity()->GetTeamNumber() != TF_TEAM_PVE_INVADERS;
...
bool CTFBotVision::IsVisibleEntityNoticed( CBaseEntity *subject ) const
{
	...
	if ( subject->IsPlayer() && me->IsEnemy( subject ) )
	{
		...
		if ( TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode())	// in MvM mode, forget spies as soon as they are fully disguised
		{
