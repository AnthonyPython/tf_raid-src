...
void CTFBot::Touch( CBaseEntity *pOther )
{
	...
	if ( them && IsEnemy( them ) )
	{
		if ( them->m_Shared.IsStealthed() || them->m_Shared.InCond( TF_COND_DISGUISED ) )
		{
			...
			if ( TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode())	// we have to build up to knowing that they are a spy in MvM
			{
				SuspectSpy( them );
			}
			else
...
void CTFBot::AvoidPlayers( CUserCmd *pCmd )
{
	...
	if ( TFGameRules() && TFGameRules()->IsMannVsMachineMode() || (TFGameRules() && TFGameRules()->IsRaidMode() ))
	{
...
void CTFBot::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode())
	{
		...
		{
			CUtlVector< CTFPlayer * > playerVector;
			CollectPlayers(&playerVector, TFGameRules()->IsRaidMode() ? TF_TEAM_PVE_DEFENDERS : TF_TEAM_PVE_INVADERS, COLLECT_ONLY_LIVING_PLAYERS);
...
float CTFBot::GetDesiredAttackRange( void ) const
{
	...
	if ( myWeapon->IsWeapon( TF_WEAPON_ROCKETLAUNCHER ) && !TFGameRules()->IsMannVsMachineMode() && (TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_BLUE))
	{
		return 1250.0f;
	}
