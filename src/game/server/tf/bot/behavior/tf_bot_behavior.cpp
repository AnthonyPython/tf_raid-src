...
void CTFBotMainAction::FireWeaponAtEnemy( CTFBot *me )
{
	...
	if ( (TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode()) && !me->IsPlayerClass( TF_CLASS_SNIPER ) && me->IsHitScanWeapon( myWeapon ) )
	{
	...
	if ( me->GetBodyInterface()->IsHeadAimingOnTarget() && threatRange < me->GetMaxAttackRange() )
	{
		...
		else if ( WeaponID_IsSniperRifle( myWeapon->GetWeaponID() ) )
		{
			...
			{
				const float reactionTime = (TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode()) ? 0.5f : 0.1f;	// just a moment to stop headshots when obviously panning too fast to see
