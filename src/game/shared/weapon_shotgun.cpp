...
ConVar sk_plr_dmg_buckshot_spas("sk_plr_dmg_buckshot_spas", "9", FCVAR_REPLICATED);

extern ConVar sk_auto_reload_time;
extern ConVar sk_plr_num_shotgun_pellets;

class CWeaponShotgun : public CBaseHDTFCombatWeapon
{
public:
	DECLARE_CLASS(CWeaponShotgun, CBaseHDTFCombatWeapon);
...
bool CWeaponShotgun::StartReload()
{
	...
	if (m_iClip1 == 0)
	{
		...
		m_flNextPrimaryAttack = gpGlobals->curtime + SequenceDuration(ACT_VM_RELOAD_EMPTY);
		m_bIsIronsighted = false;
...
bool CWeaponShotgun::Reload()
{
	...
	/// I HATE THIS SO FUCKING MUCH GOD I FUCKING HATE IT UGH
	/// all of this shit to just smoothly transfer from the reload empty acvitvity
	/// To the reload activity
	/// I barely know myself what the fuck i was doing here
	/// i fucking hate myself 
	/// 
	/// enjoy figguring something out i barrely know myself
	/// And i fucking wrote it lol
	/// -Nbc66
	if (GetActivity() == ACT_SHOTGUN_RELOAD_START && m_iClip1 == (GetMaxClip1() - 1) || m_bReloadEdgeCase)
	{
		if (m_iClip1 != GetMaxClip1() && m_iClip1 != 0 || (m_bReloadEdgeCase && m_flNextPrimaryAttack <= gpGlobals->curtime))
		{
		...
		else if(m_bReloadEdgeCase)
		{
			SendWeaponAnim(ACT_VM_RELOAD_EMPTY);
			pOwner->m_flNextAttack = gpGlobals->curtime;
			m_flNextPrimaryAttack = gpGlobals->curtime + SequenceDuration() - 0.01f;	
			FillClip();
			return true;
		}
	}
...
void CWeaponShotgun::FinishReload()
{
	...
	if (m_bReloadEdgeCase)
	{
		m_bReloadEdgeCase = false;
	}
...
void CWeaponShotgun::PrimaryAttack()
{
	...
	info.m_flDamage = sk_plr_dmg_buckshot_spas.GetFloat();
...
void CWeaponShotgun::ItemPostFrame()
{
	...
	//Yup this sucks
	//-Nbc66	
	if (pOwner->GetAmmoCount(m_iPrimaryAmmoType) == 0 && (pOwner->m_nButtons & IN_ATTACK) != 0 && m_flNextPrimaryAttack <= gpGlobals->curtime && m_iClip1 == 0)
	{
		DryFire();
	}
	...
	if (m_bInReload)
	{
		...
		if ((pOwner->m_nButtons & IN_ATTACK) != 0 && m_iClip1 >= 1 && m_flNextPrimaryAttack <= gpGlobals->curtime)
		{
			//Added for flair
			//-Nbc66
			FinishReload();
