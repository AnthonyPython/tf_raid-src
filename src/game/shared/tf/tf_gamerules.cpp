...
//We can't see it in console nor any where else why notify at all, especially when it is done often as it is.
...
ConVar tf_powerup_mode( "tf_powerup_mode", "0", FCVAR_NOTIFY, "Enable/disable powerup mode. Not compatible with Mann Vs Machine mode or Raid mode", cc_powerup_mode );
...
float CTFGameRules::GetRespawnWaveMaxLength( int iTeam, bool bScaleWithNumPlayers )
{
	...
	if ( IsMannVsMachineMode() || IsRaidMode())
	{
		bScale = false;
	}
	...
	if ( pVictimBaseEntity && pVictimBaseEntity->m_takedamage != DAMAGE_EVENTS_ONLY && pVictim )
	{
		...
		if ( !iPierceResists )
		{
			...
			if ( (info.GetDamageType() & (DMG_BLAST) ) )
			{
				...
				if( pAttacker != pVictimBaseEntity || IsMannVsMachineMode() || IsRaidMode())
				{
					bReduceBlast = true;
				}
...
void CTFGameRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	...
	if ( pObject )
	{
		if ( pObject->GetBuilder() != pVictim )
		{
			...
			if ( (IsMannVsMachineMode() || IsRaidMode() ) && pVictim && pVictim->IsPlayer() )
			{
...
// 						}
	...
	if ( event )
	{
		...
		if (pTFPlayerVictim->IsSpecial())
		{
			iDeathFlags |= TF_DEATH_SPECIAL;
		}
...
bool	ScriptIsRaidMode() { return TFGameRules()->IsRaidMode(); }
