...
const char* CTFPlayerShared::GetSoldierBuffEffectName( void )
{
	...
	{
		if (TFGameRules()->IsMannVsMachineMode() && m_pOuter->GetTeamNumber() == TF_TEAM_BLUE )
		{
			// MVM robot version has fewer particles. Helps keep the framerate up.
			return "soldierbuff_mvm";
		}
		else if (TFGameRules()->IsRaidMode() && m_pOuter->GetTeamNumber() == TF_TEAM_RED)
		{
			// MVM robot version has fewer particles. Helps keep the framerate up.
			return "soldierbuff_mvm";
		}
...
void CTFPlayer::FireBullet( CTFWeaponBase *pWpn, const FireBulletsInfo_t &info, bool bDoEffects, int nDamageType, int nCustomDamageType /*= TF_DMG_CUSTOM_NONE*/ )
{
	...
	if ( bPenetratingShot )
	{
		...
		FOR_EACH_VEC( vecTracedEntities, i )
		{
			...
			if ( ( TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode()) && iChargedPenetration == 0 )
			{
...
float CTFPlayer::TeamFortress_CalculateMaxSpeed( bool bIgnoreSpecialAbility /*= false*/ ) const
{
	...
	if ( ( !TFGameRules()->IsMannVsMachineMode() && !TFGameRules()->IsRaidMode()) || !IsMiniBoss() ) // No aiming slowdown penalties for MiniBoss players in MVM
	{
...
const char *CTFPlayer::GetOverrideStepSound( const char *pszBaseStepSoundName )
{
	...
	if( ( (TFGameRules() && TFGameRules()->IsMannVsMachineMode() && GetTeamNumber() == TF_TEAM_PVE_INVADERS) || (TFGameRules() && TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)) && !IsMiniBoss() && !m_Shared.InCond(TF_COND_DISGUISED) )
	{
		return "MVM.BotStep";
	}
...
bool CTFPlayer::IsSpecial(void) const
{
	return m_bIsSpecial;
}
