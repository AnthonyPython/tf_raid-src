...
private:
	...
	inline bool IsAttachedToStrider( ) const;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::Precache( )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::Spawn( )
{
	SetModelName( AllocPooledString( "models/magnusson_device.mdl" ) );
	BaseClass::Spawn( );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::Activate( )
{
...
void CWeaponStriderBuster::OnRestore( )
{
	...
	if( m_hConstrainedEntity != NULL )
	{
		...
		if( pStrider != NULL )
		{
			...
			if( !CreateConstraintToObject( pBoneFollower ) )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::DestroyConstraint( )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::UpdateOnRemove( )
{
	DestroyConstraint( );
...
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::CreateDestroyedEffect( )
{
	CBaseEntity *pTrail;
...
void CWeaponStriderBuster::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if( ( VPhysicsGetObject( )->GetGameFlags( ) & FVPHYSICS_PLAYER_HELD ) != NULL )
	{
		BaseClass::VPhysicsCollision( index, pEvent );
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
inline bool CWeaponStriderBuster::IsAttachedToStrider( ) const
{
	CBaseEntity *pAttachedEnt = GetOwnerEntity( );
	if( pAttachedEnt != NULL && FClassnameIs( pAttachedEnt, "npc_strider" ) )
		return true;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::Detonate( )
{
	CBaseEntity *pVictim = GetOwnerEntity( );
	if( !m_bDud && pVictim != NULL )
	{
...
int CWeaponStriderBuster::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if( GetParent( ) != NULL && GetParent( )->ClassMatches( g_iszVehicle ) )
		return 0;
	...
	if( pAttacker != NULL && ( pAttacker->Classify( ) == CLASS_COMBINE || pAttacker->Classify( ) == CLASS_COMBINE_HUNTER ) )
		if( VPhysicsGetObject( ) != NULL && !VPhysicsGetObject( )->IsMoveable( ) )
			return 0;
	...
	if( hunter_hate_held_striderbusters.GetBool( ) || hunter_hate_thrown_striderbusters.GetBool( ) || hunter_hate_attached_striderbusters.GetBool( ) )
	{
		if( GetHealth( ) > 0 && pInflictor != NULL && FClassnameIs( pInflictor, "hunter_flechette" ) )
		{
			...
			//
			if( IsAttachedToStrider( ) || ( info.GetDamageType( ) & DMG_NEVERGIB ) == 0 )
			{
	...
	if( ( info.GetDamageType( ) & DMG_CRUSH ) != 0 )
...
void CWeaponStriderBuster::BusterFlyThink( )
{
	...
	if( magradius > 0 && GetMoveType( ) == MOVETYPE_VPHYSICS && VPhysicsGetObject( ) != NULL )
	{
		...
		for( int ii = 0; ii < count; ++ii )
		{
			...
			if( pStrider != NULL && !pStrider->CarriedByDropship( ) ) // ShouldStickToEntity() doesn't work because the strider NPC isn't what we glue to
			{
...
void CWeaponStriderBuster::OnFlechetteAttach( Vector &vecFlechetteVelocity )
{
	...
	if( GetParent( ) == NULL || !GetParent( )->ClassMatches( g_iszVehicle ) )
	{
		if( !m_bNoseDiving )
		{
			...
			if( ( m_iBusterFlags & STRIDERBUSTER_FLAG_KNOCKED_OFF_STRIDER ) != NULL )
				DispatchParticleEffect( "striderbuster_shotdown_trail", PATTACH_ABSORIGIN_FOLLOW, this );
			else
