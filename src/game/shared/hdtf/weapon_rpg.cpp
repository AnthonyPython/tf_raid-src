...
static ConVar rpg_missle_use_custom_detonators( "rpg_missle_use_custom_detonators", "1" );
...
{
	if( ( info.GetDamageType( ) & ( DMG_MISSILEDEFENSE | DMG_AIRBOAT ) ) == 0 )
		return 0;
...
void CMissile::IgniteThink( )
{
	...
	if( m_hOwner != NULL && m_hOwner->GetOwner( ) )
	{
...
void CWeaponRPG::UpdateLaserPosition( Vector vecMuzzlePos, Vector vecEndPos )
{
	...
	if( m_hLaserDot != NULL )
	{
		...
		if( tr.DidHitNonWorldEntity( ) )
		{
			...
			if( pHit != NULL && pHit->m_takedamage != 0 )
...
void CWeaponRPG::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	...
	case EVENT_WEAPON_SMG1:
	{
		...
		CMissile *missile = CMissile::Create(muzzlePoint, vecAngles, GetOwner()->edict());
		missile->m_hOwner = this;
		missile->SetGracePeriod(0.5);
		missile->SetDamage(200.f);
