...
CSinglePlayerAnimState::CSinglePlayerAnimState( CBasePlayer *pPlayer ) :
	m_pPlayer( pPlayer )
{
	m_flGaitYaw = 0.0f;
...
void CSinglePlayerAnimState::EstimateYaw( )
{
	...
	{
		m_flGaitYaw = atan2( est_velocity[1], est_velocity[0] ) * 180.0f / M_PI_F;

		if( m_flGaitYaw > 180.0f )
			m_flGaitYaw = 180.0f;
		else if( m_flGaitYaw < -180.0f )
			m_flGaitYaw = -180.0f;
	}
}
...
void CSinglePlayerAnimState::ComputePoseParam_BodyPitch( CStudioHdr *pStudioHdr )
{
	...
	flPitch = Clamp( flPitch, -90.0f, 90.0f );
	...
	if(GetBasePlayer()->LookupPoseParameter("aim_pitch") != -1) // RAY: Fixed assert
...
void CSinglePlayerAnimState::ComputePoseParam_BodyLookYaw( )
{
	QAngle absangles = GetBasePlayer( )->GetAbsAngles( );
