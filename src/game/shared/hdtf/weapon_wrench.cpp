...
ConVar sk_plr_dmg_wrench_primary("sk_plr_dmg_wrench_primary", "15", FCVAR_REPLICATED);
ConVar sk_plr_dmg_wrench_secondary("sk_plr_dmg_wrench_secondary", "30", FCVAR_REPLICATED);
ConVar sk_npc_dmg_wrench_primary("sk_npc_dmg_wrench_primary", "15", FCVAR_REPLICATED);
ConVar sk_npc_dmg_wrench_secondary("sk_npc_dmg_wrench_secondary", "30", FCVAR_REPLICATED);

class CWeaponWrench : public CBaseMeleeWeapon
{
public:
	DECLARE_CLASS(CWeaponWrench, CBaseMeleeWeapon);
...
float CWeaponWrench::GetDamageForActivity(Activity hitActivity)
{
	...
	if (pOwner != NULL && pOwner->IsPlayer())
		return (hitActivity == GetPrimaryAttackActivity() ? sk_plr_dmg_wrench_primary : sk_plr_dmg_wrench_secondary).GetFloat();

	return (hitActivity == GetPrimaryAttackActivity() ? sk_npc_dmg_wrench_primary : sk_npc_dmg_wrench_secondary).GetFloat();
}

//-----------------------------------------------------------------------------
// Purpose: Add in a view kick for this weapon
//-----------------------------------------------------------------------------
void CWeaponWrench::AddViewKick()
{
