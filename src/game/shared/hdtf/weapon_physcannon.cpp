...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if( m_pPlayer != NULL )
	{
		...
		if( m_pPlayer->GetActiveWeapon( ) != NULL && !m_pPlayer->GetActiveWeapon( )->Deploy( ) )
...
void CPlayerPickupController::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if( ToBasePlayer( pActivator ) == m_pPlayer )
	{
		...
		if( ( m_pPlayer->m_nButtons & IN_ATTACK ) != 0 )
		{
			Shutdown( true );
			Vector vecLaunch;
...
public:
	...
	CMaterialReference &GetMaterial( )
	{
		return m_hMaterial;
	}

	CInterpolatedValue &GetAlpha( )
	{
		return m_Alpha;
	}

	CInterpolatedValue &GetScale( )
	{
		return m_Scale;
	}
...
class CWeaponPhysCannon : public CBaseHDTFCombatWeapon
{
public:
	DECLARE_CLASS( CWeaponPhysCannon, CBaseHDTFCombatWeapon );
...
protected:
	...
	void StartEffects( );	// Initialize all sprites and beams
	void StopEffects( bool stopSound = true );	// Hide all effects temporarily
	void DestroyEffects( );	// Destroy all sprites and beams
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::SetViewModel( )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponPhysCannon::CanHolster() const
{
...
void CWeaponPhysCannon::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	if( Pickup_OnAttemptPhysGunPickup( pEntity, pOwner, PUNTED_BY_CANNON ) )
	{
		...
		if( ( pList[0]->GetGameFlags( ) & FVPHYSICS_CONSTRAINT_STATIC ) != 0 && pEntity->GetServerVehicle( ) )
			forward.Init( );
...
void CWeaponPhysCannon::SecondaryAttack( )
{
	...
	if( m_bActive && ( pOwner->m_afButtonPressed & IN_ATTACK2 ) != 0 )
	{
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject( )
{
	...
	if( tr.fraction != 1.0f && tr.m_pEnt != NULL && tr.m_pEnt->IsWorld( ) == false )
	{
...
CBaseEntity *CWeaponPhysCannon::FindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone )
{
	...
	for( int i = 0; i < count; ++i )
	{
		if( list[i]->VPhysicsGetObject( ) == NULL )
			continue;
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	...
	if( pObject != NULL && m_bResetOwnerEntity == true )
...
void CWeaponPhysCannon::ManagePredictedObject( )
{
	...
	{
		if( m_hOldAttachedObject != NULL && m_hOldAttachedObject->VPhysicsGetObject( ) )
		{
...
void CWeaponPhysCannon::ItemPreFrame( )
{
	...
	if( localplayer != NULL && !localplayer->IsObserver( ) )
...
bool CWeaponPhysCannon::CanPickupObject( CBaseEntity *pTarget )
{
	...
	if( pTarget->GetBaseAnimating( ) != NULL && pTarget->GetBaseAnimating( )->IsDissolving( ) )
		return false;
	...
	CBasePlayer *pOwner = ToBasePlayer( GetOwner( ) );
	if( pOwner != NULL && pOwner->GetGroundEntity( ) == pTarget )
		return false;
	...
	if( pObj != NULL && pObj->GetGameFlags( ) & FVPHYSICS_PLAYER_HELD )
		return false;
...
void CWeaponPhysCannon::StartEffects( )
{
	...
	for( int i = PHYSCANNON_GLOW1; i < PHYSCANNON_GLOW1 + NUM_GLOW_SPRITES; ++i )
	{
		...
		if( !m_Parameters[i].SetMaterial( PHYSCANNON_GLOW_SPRITE ) )
			// This means the texture was not found
			Assert( 0 );
	}
	...
	for( int i = PHYSCANNON_ENDCAP1; i < PHYSCANNON_ENDCAP1 + NUM_ENDCAP_SPRITES; ++i )
	{
		...
		if( !m_Parameters[i].SetMaterial( PHYSCANNON_ENDCAP_SPRITE ) )
			// This means the texture was not found
			Assert( 0 );
	}
...
int CWeaponPhysCannon::DrawModel( int flags )
{
	...
	if( ( flags & STUDIO_TRANSPARENCY ) != 0 )
	{
		DrawEffects( );
		return 1;
	}
...
//-----------------------------------------------------------------------------
bool CWeaponPhysCannon::IsTransparent( )
{
	return true;
}
...
bool PlayerPickupControllerIsHoldingEntity( CBaseEntity *pPickupControllerEntity, CBaseEntity *pHeldEntity )
{
	...
	return pController != NULL ? pController->IsHoldingEntity( pHeldEntity ) : false;
}
...
CBaseEntity *GetPlayerHeldEntity( CBasePlayer *pPlayer )
{
	...
	if( pPlayerPickupController != NULL )
