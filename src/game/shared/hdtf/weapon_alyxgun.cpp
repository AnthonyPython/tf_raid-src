...
int CWeaponAlyxGun::WeaponRangeAttack1Condition( float flDot, float flDist )
{
	...
	if( flDist < m_fMinRange1 )
	{
		...
		float flTime = gpGlobals->curtime - m_flTooCloseTimer;
	...
	{
		Vector vecRoughDirection = pOwner->GetEnemy( )->WorldSpaceCenter( ) - WorldSpaceCenter( );
		Vector vecRight;
