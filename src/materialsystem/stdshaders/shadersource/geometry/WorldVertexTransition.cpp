...
std::function<void(IShaderDynamicAPI*, int, int, bool, int, bool, bool)> DeclareWVTDynamics[]
{
...
/*s14*/			EnableSampler(SAMPLER_SELFILLUM2, true)
	...
	else // End of Snapshotting ------------------------------------------------------------------------------------------------------------------------------------------------------------------
	{
		...
		// For some reason Hammer Blendfactor is inverted so we have to invert texture binding.
		// TODO: Find whoever caused this and force them to use their own map editor
		if (pShaderAPI->InEditorMode())
		{
/*s0*/		BindTextureWithCheckAndFallback(bHasBaseTexture, SAMPLER_BASETEXTURE2, info.m_nBaseTexture, info.m_nBaseTextureFrame, TEXTURE_WHITE)
/*s1*/		BindTextureWithCheckAndFallback(bHasBaseTexture2, SAMPLER_BASETEXTURE, info.m_nBaseTexture2, info.m_nBaseTextureFrame2, TEXTURE_WHITE)

				if (bHasAnyNormalTexture)
				{
/*s2*/			BindTextureWithCheckAndFallback(bHasNormalTexture, SAMPLER_NORMALTEXTURE2, info.m_nBumpMap, info.m_nBumpFrame, TEXTURE_NORMALMAP_FLAT)
/*s3*/			BindTextureWithCheckAndFallback(bHasNormalTexture2, SAMPLER_NORMALTEXTURE, info.m_nBumpMap2, info.m_nBumpFrame2, TEXTURE_NORMALMAP_FLAT)
/*s11*/			BindTextureStandard(SAMPLER_LIGHTMAP, TEXTURE_LIGHTMAP_BUMPED)
				}
				else
				{
/*s11*/			BindTextureStandard(SAMPLER_LIGHTMAP, TEXTURE_LIGHTMAP)
				}
#ifdef DETAILTEXTURING
				// DO NOT FLIP
/*s4*/			BindTextureWithCheck(bHasDetailTexture, SAMPLER_DETAILTEXTURE, info.m_nDetailTexture, info.m_nDetailFrame)
/*s10*/			BindTextureWithCheck(bHasDetailTexture2, SAMPLER_DETAILTEXTURE2, info.m_nDetailTexture2, info.m_nDetailFrame2)
#endif
				if (bSelfIllum)
				{
					if (bHasAnySelfIllumMask)
					{
/*s13*/				BindTextureWithCheckAndFallback(bHasSelfIllumMask, SAMPLER_SELFILLUM2, info.m_nSelfIllumMask, info.m_nSelfIllumMaskFrame, TEXTURE_BLACK)
/*s14*/				BindTextureWithCheckAndFallback(bHasSelfIllumMask2, SAMPLER_SELFILLUM, info.m_nSelfIllumMask2, info.m_nSelfIllumMaskFrame2, TEXTURE_BLACK)
					}
#ifdef SELFILLUMTEXTURING
					else if (bHasAnySelfIllumTexture)
					{
/*s13*/				BindTextureWithCheckAndFallback(bHasSelfIllumTexture, SAMPLER_SELFILLUM2, info.m_nSelfIllumTexture, info.m_nSelfIllumTextureFrame, TEXTURE_BLACK)
/*s14*/				BindTextureWithCheckAndFallback(bHasSelfIllumTexture2, SAMPLER_SELFILLUM, info.m_nSelfIllumTexture2, info.m_nSelfIllumTextureFrame2, TEXTURE_BLACK)
					}
#endif
				}
			...
			if (bHasAnyEnvMapMask)
			{
/*s2*/			BindTextureWithCheckAndFallback(bHasEnvMapMask, SAMPLER_ENVMAPMASK2, info.m_nEnvMapMask, info.m_nEnvMapMaskFrame, TEXTURE_BLACK)
/*s3*/			BindTextureWithCheckAndFallback(bHasEnvMapMask2, SAMPLER_ENVMAPMASK, info.m_nEnvMapMask2, info.m_nEnvMapMaskFrame2, TEXTURE_BLACK)
			}
		}
		else // ELSE = NOT USING HAMMER MAP CORRUPTOR
		{
/*s0*/		BindTextureWithCheckAndFallback(bHasBaseTexture, SAMPLER_BASETEXTURE, info.m_nBaseTexture, info.m_nBaseTextureFrame, TEXTURE_WHITE)
/*s1*/		BindTextureWithCheckAndFallback(bHasBaseTexture2, SAMPLER_BASETEXTURE2, info.m_nBaseTexture2, info.m_nBaseTextureFrame2, TEXTURE_WHITE)

			if (bHasAnyNormalTexture)
			{
/*s2*/			BindTextureWithCheckAndFallback(bHasNormalTexture, SAMPLER_NORMALTEXTURE, info.m_nBumpMap, info.m_nBumpFrame, TEXTURE_NORMALMAP_FLAT)
/*s3*/			BindTextureWithCheckAndFallback(bHasNormalTexture2, SAMPLER_NORMALTEXTURE2, info.m_nBumpMap2, info.m_nBumpFrame2, TEXTURE_NORMALMAP_FLAT)
/*s11*/			BindTextureStandard(SAMPLER_LIGHTMAP, TEXTURE_LIGHTMAP_BUMPED)
			}
			else
			{
...
/*s10*/			BindTextureWithCheck(bHasDetailTexture2, SAMPLER_DETAILTEXTURE2, info.m_nDetailTexture2, info.m_nDetailFrame2)
			...
			if (bSelfIllum)
			{
				if (bHasAnySelfIllumMask)
				{
/*s13*/				BindTextureWithCheckAndFallback(bHasSelfIllumMask, SAMPLER_SELFILLUM, info.m_nSelfIllumMask, info.m_nSelfIllumMaskFrame, TEXTURE_BLACK)
/*s14*/				BindTextureWithCheckAndFallback(bHasSelfIllumMask2, SAMPLER_SELFILLUM2, info.m_nSelfIllumMask2, info.m_nSelfIllumMaskFrame2, TEXTURE_BLACK)
				}
#ifdef SELFILLUMTEXTURING
				else if (bHasAnySelfIllumTexture)
				{
/*s13*/				BindTextureWithCheckAndFallback(bHasSelfIllumTexture, SAMPLER_SELFILLUM, info.m_nSelfIllumTexture, info.m_nSelfIllumTextureFrame, TEXTURE_BLACK)
/*s14*/				BindTextureWithCheckAndFallback(bHasSelfIllumTexture2, SAMPLER_SELFILLUM2, info.m_nSelfIllumTexture2, info.m_nSelfIllumTextureFrame2, TEXTURE_BLACK)
				}
#endif
			}
			...
			if (bHasAnyEnvMapMask)
			{
/*s2*/			BindTextureWithCheckAndFallback(bHasEnvMapMask, SAMPLER_ENVMAPMASK, info.m_nEnvMapMask, info.m_nEnvMapMaskFrame, TEXTURE_BLACK)
/*s3*/			BindTextureWithCheckAndFallback(bHasEnvMapMask2, SAMPLER_ENVMAPMASK2, info.m_nEnvMapMask2, info.m_nEnvMapMaskFrame2, TEXTURE_BLACK)
			}
		}
		...
		if (bHasEnvMap)
		{
/*s15*/		BindTextureWithCheckAndFallback(mat_specular.GetBool(), SAMPLER_ENVMAPTEXTURE, info.m_nEnvMap, info.m_nEnvMapFrame, TEXTURE_BLACK)

		}
#endif

#ifdef LIGHTWARP
/*s6*/	BindTextureWithCheck(bHasLightWarpTexture, SAMPLER_LIGHTWARP, info.m_nLightWarpTexture, 0)
#endif

/*s12*/ BindTextureWithCheck(bHasBlendModulateTexture, SAMPLER_BLENDMODULATE, info.m_nBlendModulateTexture, info.m_nBlendModulateFrame)
