...
void StaticShader_LuxVLG_Simple(IShaderShadow *pShaderShadow,
	...
	bool bSeamlessBase, int iDetailUV, int iBumpUV, int iEnvMapMaskUV)
{
...
void DynamicShader_LuxVLG_Simple(IShaderDynamicAPI *pShaderAPI, int ifogIndex, int iLightingPreview, bool bFlashlightShadows,
	...
	int iBones, int iVertexCompression, bool bStaticPropLighting, bool bDynamicPropLighting, bool bAmbientLight)
{
...
//	SET_DYNAMIC_VERTEX_SHADER_COMBO(NUM_LIGHTS, iNumLights);
...
//	SET_DYNAMIC_VERTEX_SHADER_COMBO(NUM_LIGHTS, 0); // iNumLights... This isn't even necessary!
	SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING, iBones);
	SET_DYNAMIC_VERTEX_SHADER_COMBO(STATICPROPLIGHTING, bStaticPropLighting);
	SET_DYNAMIC_VERTEX_SHADER_COMBO(DYNAMICPROPLIGHTING, !bStaticPropLighting && bDynamicPropLighting);
	SET_DYNAMIC_VERTEX_SHADER(lux_model_vs30);
...
std::function<void(IShaderShadow*, bool, bool, bool, int, int, bool, bool, bool, bool, bool, int, bool, bool, int, int, int)> DeclareStatics[]
{
...
std::function<void(IShaderDynamicAPI*, int, int, bool, int, bool, bool, bool, int, int, int, bool, bool, bool)> DeclareDynamics[]
{
...
void LuxVertexLitGeneric_Init_Params(CBaseVSShader *pShader, IMaterialVar **params, const char *pMaterialName, Model_Vars_t &info)
{
	...
	if (params[info.m_nBumpMap]->IsDefined())
	{
		...
		if (params[info.m_nLightMap]->IsDefined())
		{
	...
	else
	{
		...
		if (params[info.m_nNormalTexture]->IsDefined() && !params[info.m_nLightMap]->IsDefined())
		{
	...
	// Default values are now 1.0f, 1.0f, 0.0f and thats that.
	...
	// 13.02.2023 ShiroDkxtro2 : 
	// We don't want the phong parameter to be... real...
	// If the thing even just *exists*, the engine will just kill model lightmapping.
	// So I did something quite ingenius, if I can say so myself hehe
	// Int, Float, bool, color params... They all have default values of 0 or 1
	// String params? They don't. You don't have something on them and they are just empty strings ("")
	// The idea behind this is that both $bumpmap "" will not cause the engine to do its thing. So the same should apply to phong.
	// This if statement checks if our Phong param is NOT "" and NOT "0". strncmp returns 0 if the comparison results true ( Yeah doesn't make sense does it :/? )
	if (GetIntParamValue(info.m_nPhong) != 0) // strncmp(params[info.m_nFakePhongParam]->GetStringValue(), "", 1) && strncmp(params[info.m_nFakePhongParam]->GetStringValue(), "0", 1)
	{
...
/*	if
 ...
 (!params[info.m_nEnvMap]->GetTextureValue()->IsError())
	{
	...
	CBasePerMaterialContextData **pContextDataPtr)
{
	...
	//==================================================================================================
#ifdef PHONG_REFLECTIONS
	bool	bHasPhong					=	bHasNormalTexture	&&	GetIntParamValue(info.m_nPhong) != 0; // Can only use Phong if you have a NormalTexture
	...
	if (bHasPhong)
	{
		...
		// Do we have to reproduce the old behaviour?
		if (!bHasPhongNewBehaviour)
		{
			// Not using $NormalMapAlphaEnvMapMask will set the BaseTexture Alpha as the EnvMapMask
			if (bNormalMapAlphaEnvMapMask)
			{
				bHasEnvMap = !bHasFlashlight; // Stock Phong has EnvMapping.
				bHasEnvMapFresnel = !bHasFlashlight; // Stock Phong has EnvMapFresnel from the PhongFresnelRanges.

				if (!bHasFlashlight) // REALLY important! You will have square Flashlight without this.
				{
	...
	if (pShader->IsSnapshotting())
	{
		...
		EnableSamplerWithCheck((bHasNormalTexture || bHasLightWarpTexture), SAMPLER_NORMALTEXTURE, false)	// No sRGB
#else
		...
		DeclareStaticShaders(pShaderShadow, bHasDetailTexture, bHasEnvMapMask,
											...
											bHasSeamlessBase, iModeDetail, iModeBump, iModeEnvMapMask);
	}
	...
	DeclareDynamic(pShaderAPI, iFogIndex, 0, bFlashlightShadows, (pShaderAPI->GetPixelFogCombo()), bWriteDepthToAlpha,
		...
		(int)vertexCompression, bHasStaticPropLighting, bHasDynamicPropLighting, LightState.m_bAmbientLight);
