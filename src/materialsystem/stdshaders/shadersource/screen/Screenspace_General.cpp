...
//
//	Purpose of this File :	Handle any incoming requests for post processing.
//
//===========================================================================//
...
void LuxScreenspace_Shader_Draw(CBaseVSShader *pShader, IMaterialVar **params, IShaderDynamicAPI *pShaderAPI, IShaderShadow *pShaderShadow, Screenspace_Vars_t &info, VertexCompressionType_t vertexCompression, CBasePerMaterialContextData **pContextDataPtr)
{
	...
	if (pShader->IsSnapshotting())
	{
		...
		if (bHasBaseTexture)
		{
			...
			if (BaseTextureIF == IMAGE_FORMAT_RGBA16161616F || BaseTextureIF == IMAGE_FORMAT_RGBA16161616)
				...
				Warning("Screenspace Basetexture = %s", params[info.m_nBaseTexture]->GetStringValue());
		}
		...
		if (bHasTexture1)
		{
			...
			if (Texture1IF == IMAGE_FORMAT_RGBA16161616F || Texture1IF == IMAGE_FORMAT_RGBA16161616)
				...
				Warning("Screenspace Texture1 = %s", params[info.m_nTexture1]->GetStringValue());
		}
		...
		if (bDoAlphaBlending)
		{
			DECLARE_STATIC_PIXEL_SHADER(lux_post_alphablend_ps30);
			SET_STATIC_PIXEL_SHADER(lux_post_alphablend_ps30);
		}
		else
		{
	...
	else // Dynamic state starts here.
	{
		...
		if (bDoAlphaBlending)
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(lux_post_alphablend_ps30);
			SET_DYNAMIC_PIXEL_SHADER(lux_post_alphablend_ps30);
		}
		...
		BindTextureWithCheck(bHasTexture1,		SHADER_SAMPLER1,	info.m_nTexture1, 0)
		BindTextureWithCheck(bHasTexture2,		SHADER_SAMPLER2,	info.m_nTexture2, 0)
		BindTextureWithCheck(bHasTexture3,		SHADER_SAMPLER3,	info.m_nTexture3, 0)
	}
